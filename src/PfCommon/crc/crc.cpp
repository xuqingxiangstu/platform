...
namespace Pf
{
    namespace PfCommon
    {
        ...
        unsigned short Crc::calCrc16(const unsigned char *pbDataBuf, unsigned int dwNumOfBytes)
        {
            unsigned short *pwCRCTable = crc16_table;

            unsigned char	bData = 0;
            unsigned short	wCRC = 0x0000;	// CRC校验码

            while (0 != (dwNumOfBytes--))
            {
                //		*pbDataBuf = (*pbDataBuf)&0xFF;
                bData = (wCRC >> 8) & 0xFF;				// 以8位二进制数的形式暂存wCRC的高8位
                wCRC = (wCRC << 8) & 0xFFFF;
                //b[i] = bData ^ (*pbDataBuf);			// 左移8位，相当于wCRC的低8位乘以2的8次方
        ...
        unsigned char Crc::calSum(const unsigned char *DataBuf, unsigned int dwNumOfBytes)
        {
