...
namespace Pf
{
    namespace PfIcdWorkBench
    {
        ...
        void frameJG::init(const TiXmlElement *ele)
        {

        }

        std::shared_ptr<frameObj>  frameJG::clone()
        {
            frameJG *obj = new frameJG();
        ...
        void frameJG::resendMsg(byteArray &outValue)
        {

        }

        //通用仿真
        void frameJG::simulation(byteArray &outValue, const std::string &json)
        {
            ...
            bool isOk;
            unsigned int infoType = tmpValue.toUInt(&isOk, 16);

            unsigned int frameType = 0;
            frameType = (src << 24) + (dst << 16) + infoType;
            ...
            //帧计数
            unsigned char cmdCnt = frameNumberManager::getInstance()->getFrameNumber(mCurUuid, getFrameName(), src, dst);
            ...
            //校验和
            outValue[15] = Pf::PfCommon::Crc::calSum((const unsigned char*)&outValue.at(0), outValue.size());
        }

        void frameJG::_fillRegion(byteArray &outValue, const Json::Value &regionJs)
        {
            if(regionJs.isNull())
                return ;
        ...
        //自定义仿真
        void frameJG::simulation(byteArray &outValue, const Json::Value &json)
        {
        ...
        bool frameJG::parse(unsigned char *u8Msg, const unsigned int u32Size, Json::Value &result)
        {
            if(u32Size < 2)
                return false;
            ...
            //帧类型
            byteSize = 4;
            unsigned int tmpType = data.getData(u8Msg, u32Size, pos, byteSize, 0, 0);

            pos += byteSize;

            unsigned int frameType = 0;
            frameType = tmpType & 0xFFFF;

            result["frame_type"] = frameType;

            //源节点表号
            result["src_num"] = (tmpType >> 24) & 0xFF;

            //目标节点表号
            result["dst_num"] = (tmpType >> 16) & 0xFF;
        ...
        void frameJG::_parseRegion(const unsigned int &tableNum, const unsigned char *u8Msg, const unsigned int u32Size, Json::Value &regionValue)
        {
            //从数据库中获取参数信息
            Json::Value paramValues;
            paramsTable::getInstance()->getValues(tableNum, paramValues);
