...
void czxkBusiness::exeCmd(Json::Value valueJs)
{
    ...
    //更新参数
    virtualParams::getInstance()->setValue({mDevUuid, mViratulTableNum, std::to_string(testNum)}, mapValue());
}

void czxkBusiness::testInfo(Json::Value valueJs)
{
    Json::Value regionJs = valueJs["data"];
...
void czxkBusiness::queryVersion(Json::Value regionJs)
{
...
void czxkBusiness::sendFsCmd(Json::Value regionJs)
{
    //回令
    Json::Value array;
    array.append(0xEE);
...
void czxkBusiness::checkCmd(Json::Value regionJs)
{
...
void czxkBusiness::respondCmd(int frameType, int code, Json::Value array)
{
