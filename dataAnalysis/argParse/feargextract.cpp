...
void feArgExtract::judge(const unsigned int &table, const unsigned int &coding, const Json::Value &value, const std::string &srcValue)
{
    bool findMean = false;
    std::string chMean = "";
    ...
    Json::Value confJs;
    if(paramsTable::getInstance()->getValue(QString::number(table, 16), coding, confJs))
    {
        //step1:判断是否超差
    ...
    objJs.insert(RESULT_TABLE_TABLE_NUM, QString::number(table, 16));
